...
function Portal:update(dt, fireMode, shiftHeld)
...
function Portal:discharge()
  ...
  activeItem.interact("OpenTeleportDialog", "/interface/warping/portal.config", activeItem.ownerEntityId())
  self:setState(self.cooldown)
end

function Portal:cooldown()
...
function Portal:targetValid(aimPos)
...
function Portal:createProjectile()
...
function Portal:uninit(weaponUninit)
  self:reset()
end
